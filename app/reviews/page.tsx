...
export default function ReviewsPage() {
  ...
  const filteredReviews = reviews.filter((r) => {
    ...
    const starMatch = activeStars.length === 0 || activeStars.includes(r.rating);
    return brandMatch && marketMatch && starMatch;
  });

  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      const allFilteredIds = filteredReviews.map(r => r.id);
  ...
  const handleGenerateAll = async () => {
    const pendingIds = filteredReviews.filter(r => !r.aiReply).map(r => r.id);
    if (pendingIds.length === 0) {
      alert('No reviews without AI replies in the current filter.');
      return;
    }
    ...
    try {
      ...
      while (remaining.length > 0) {
        ...
        setGenerateProgress(`Generating ${totalGenerated + totalFailed + 1}-${totalGenerated + totalFailed + batch.length} of ${pendingIds.length}...`);

        const res = await fetch('/api/reviews/generate-all', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ids: batch, limit: BATCH_SIZE }),
        });
        const data = await res.json();
      ...
      setGenerateProgress('');
      alert(`Generated ${totalGenerated} AI replies${totalFailed > 0 ? `, ${totalFailed} failed` : ''}.`);
    } catch (err) {
  ...
  const handleReplySelected = async (idsToPost: string[]) => {
    ...
    try {
      const res = await fetch('/api/reviews/reply-all', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: idsToPost }),
      });
      const data = await res.json();
      if (data.success) {
        ...
        alert(`Failed to post replies: ${data.error || 'Unknown error'}`);
      }
    } catch (err: any) {
      console.error('Reply posting error:', err);
      alert(`Error posting replies: ${err.message}`);
    } finally {
      setPosting(false);
    }
  };
      ...
      <header className="border-b border-gray-800 px-8 py-4 flex items-center justify-between bg-gray-900">
        ...
        <div className="flex items-center gap-3">
          ...
          {generating && generateProgress && (
            <span className="text-xs text-purple-400">{generateProgress}</span>
          )}
          {selectedIds.length > 0 && (
            <button
              onClick={() => handleReplySelected(selectedIds)}
              disabled={posting || syncing || generating}
            ...
            >
              <svg className={`w-4 h-4 ${posting ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
              </svg>
              {posting ? 'Posting...' : `Reply Selected (${selectedIds.length})`}
            </button>
          )}
          <button
          ...
          >
            ...
            {generating ? 'Generating...' : 'Generate All Replies'}
          </button>
          <button
            onClick={handleSync}
              ...
              return (
                ...
                >
                  <input
                    type="checkbox"
                    checked={isSelected}
                    onClick={(e) => toggleSelectOne(review.id, e)}
                    onChange={() => {}}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 flex flex-col justify-between h-fit">
            ...
            {selectedReview && (
              <div className="mb-4">
                ...
                <textarea
                  ...
                  value={selectedReview.aiReply || `Thank you for your kind review. We're delighted that you're enjoying your purchase from ${selectedReview.storeName.replace(/\s*\([^)]*\)\s*$/, '')}. Your satisfaction is our priority, and we sincerely appreciate your support. We look forward to serving you again.`}
                />
              </div>
            )}
          </div>
          ...
          {selectedReview && (
            <div className="space-y-3 mt-4">
              <button
                onClick={() => handleReplySelected([selectedReview.id])}
                disabled={posting}
              ...
              >
                {posting ? 'Posting...' : 'Approve & Reply (Reply Comment API)'}
              </button>
            </div>
          )}
        </div>
